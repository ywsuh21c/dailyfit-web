...
## ▶ 이어받는 세션 — 지금 할 일 (2026-06-15 배포 continuation)

**목표: 홈페이지를 `dailyfitai.app`에 라이브.** 코드는 끝남(빌드 green·GitHub 푸시 완료). 지금은 **배포/도메인 단계**다. Vercel UI 액션은 Michael만 가능 → 너는 guide-me 스타일 안내 + 검증·코드픽스 담당.

**Vercel 실측 상태 (2026-06-15, account `ywsuh21c`, Hobby 플랜):**
- 새 repo **`dailyfit-web` import = Michael 진행 중** (대시보드 Add New→Project→Import→Deploy). 완료 시 `dailyfit-web-xxxx.vercel.app` 생성. **첫 배포에 필수 env 0** (코드가 다 가드, `NEXT_PUBLIC_SITE_URL`은 `lib/site.ts`에 `https://dailyfitai.app` 하드코딩).
- ⭐ **`dailyfitai.app` 도메인은 이미 옛 프로젝트 `dailyfit-website-github`(repo `ywsuh21c/dailyfit-website` = v1 정적)에 붙어 있음.** DNS가 이미 Vercel을 가리킴 → **레지스트라(가비아 등) 건드릴 필요 없음.** 도메인을 새 프로젝트로 "Move"만 하면 됨.
- 건드리지 말 옛 프로젝트: `dailyfit-website-github`·`dailyfit-website`·`dailyfit`(project-bu855)·`dailyfit-cofounder-dashboard`.

**다음 액션 순서:**
1. **(Michael) `dailyfit-web` 배포 확인** — `.vercel.app` URL 살아있나. 너는 그 URL을 `curl`로 점검(홈 200 · `/writing` · `/writing/we-write-as-we-build` · `/robots.txt` · `/sitemap.xml`). 빌드 깨지면 Vercel 로그 보고 코드 픽스 후 push(원격=라이브라 자동 재배포).
2. **⚠️ GATED — 도메인 cut-over = 진짜 런칭.** **Michael 명시 승인 후에만.** 새 `dailyfit-web` 프로젝트 → **Settings → Domains** → `dailyfitai.app` 입력 → Vercel이 "다른 프로젝트가 사용 중, 옮길까?" → **Move 확인.** 즉시 전환(레지스트라 DNS 변경 0). 이 순간 `dailyfitai.app` = v2 신사이트. 되돌리기 = 도메인을 옛 프로젝트로 다시 Move(역시 즉시).
   - `www.dailyfitai.app` 변형 있으면 같이 처리(apex로 redirect).
   - 옛 `dailyfit-website-github`는 도메인만 떼이고 `.vercel.app`로 백업 유지(삭제 금지).
3. **라이브 검증** — `https://dailyfitai.app` 실접속(폰 포함): 히어로·nav·우상단 제품 버튼·`/writing` 발행글 1편·OG·robots. HARD RULE 재확인(§3: fundraise 노출 0 · 회사면 시니어 2인칭 CTA 0).
4. **(런칭 후, 계정 단계) Plausible/Sanity/Resend** — 필요 시. Plausible은 Vercel 환경변수 `NEXT_PUBLIC_PLAUSIBLE_DOMAIN=dailyfitai.app` 넣고 redeploy하면 켜짐(이미 env-gated 래퍼 구현됨).

**막히면**: Vercel 화면 스샷 받아 guide-me. 코드/설정 이슈는 직접 픽스+push.

### 📣 광고 착지 readiness (6/26 풀런치 — 검색광고 네이버+구글)
광고는 소비자(50대 본인·자녀 27~38)를 **`dailyfitai.app/product`** 로 보낸다(루트 X — 루트는 VC/언론용 Option B). `/product` 소비자 동선 완결 + UTM 처리 완료(커밋 `c873edf`). **6/26 남은 단 하나 = 스토어 실링크 플립:**
- **방법 A(코드)**: `lib/site.ts`의 `storeLinks.ios`/`.android`에 실 URL 넣고 push → 자동 재배포.
- **방법 B(Vercel env, 코드 무수정)**: Vercel → Settings → Environment Variables 에 `NEXT_PUBLIC_IOS_APP_URL` / `NEXT_PUBLIC_ANDROID_APP_URL` 추가 → Redeploy.
- 비어있는 동안 배지 = "곧 출시"(비클릭, 안전). 채우면 "다운로드" 실링크 + 착지 UTM 자동 전달(Google Play `referrer` = Play Install Referrer 귀속, App Store 쿼리 append).
- ⚠️ 라이브 전 검증: iOS App Store URL의 **숫자 app id**, Play **패키지 id**(=`kr.dailyfit.app` 추정 — 실제 확인). 두 스토어에서 실제 열리는지 클릭 테스트.
- 손대지 말 것(이미 OK·점검 완료): 고객 type-in 폼 0 · 펀딩 비노출 · 단일 오디언스.
- 상세 요건: `1. Entrepreneurship/2. Outputs/gtm-launch/2026-06-18-orc-launch-marketing-campaign/HANDOFF-ad-landing-requirements.md`
